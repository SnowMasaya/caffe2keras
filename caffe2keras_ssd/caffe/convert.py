...
from caffe.proto import caffe_pb2 as caffe
...
def preprocessPrototxt(prototxt, debug=False):
    ...
    if (debug):
        print
        'Writing preprocessed prototxt to debug.prototxt'
...
def create_model(layers, phase, input_dim, debug=False):
    ...
    for layer_nb in network:
        ...
        # other cases
            ...
            if debug:
                print
                'input shape: ' + str(input_layers[0]._keras_shape)
            ...
            if type_of_layer == 'concat':
                ...
                if pad_h + pad_w > 0:
                    input_layers = ZeroPadding2D(padding=(pad_h, pad_w),
                                                 name=name + '_zeropadding')(input_layers)
                net_node[layer_nb] = Convolution2D(nb_filter, nb_row, nb_col,
                ...
                net_node[layer_nb] = Deconvolution2D(nb_filter, nb_row, nb_col, output_shape, name=name,
                                                     subsample=(stride_h, stride_w))(input_layers)
                ...
                if (layer.pooling_param.pool == 0):  # MAX pooling
                    # border_mode = 'same'
                    border_mode = 'valid'
                    net_node[layer_nb] = MaxPooling2D(border_mode=border_mode, pool_size=(kernel_h, kernel_w),
                                                      strides=(stride_h, stride_w), name=name)(input_layers)
                    if (debug):
                        print("MAX pooling")
                ...
                net_node[layer_nb] = BatchNormalization(axis=axis, name=name)(input_layers)
                ...
                if op == 0:
                    ...
                    raise NotImplementedError('Operation with id = "' + str(
                        op) + '" of layer with type "' + type_of_layer + '" is not implemented.')
                net_node[layer_nb] = merge(input_layers, mode=merge_mode, concat_axis=axis, name=name)

            elif type_of_layer == 'permute':
                ...
                if debug:
                    ...
                    print
                    'axis'
                axis1 = int(layer.permute_param.order[0])
                axis2 = int(layer.permute_param.order[1])
                axis3 = int(layer.permute_param.order[2])
                axis4 = int(layer.permute_param.order[3])

                input_layers = Permute((axis2, axis3, axis4))(input_layers[0])
                net_node[layer_nb] = input_layers

            elif type_of_layer == 'reshape':
                ...
                if debug:
                    ...
                    print
                    'axis'
                # axis1 = layer.reshape_param.shape.dim[0]
                # axis2 = layer.reshape_param.shape.dim[1]
                axis3 = layer.reshape_param.shape.dim[2]
                semi_model = Model(input=net_node[0], output=input_layers[0])
                op_shape = semi_model.layers[-1].output_shape
                del semi_model

                interm_layer = Reshape((op_shape[-1] // axis3, axis3))(input_layers)
                net_node[layer_nb] = interm_layer
                ...
                variance = []
                for i in range(4):
                    variance.append(layer.prior_box_param.variance[i])
                semi_model = Model(input=net_node[0], output=input_layers[0])
                ip_shape = semi_model.layers[-1].output_shape
                img_size = []
                img_size.append(int(ip_shape[1]))
                img_size.append(int(ip_shape[2]))
                del semi_model
                ...
                net_node[layer_nb] = PriorBox(img_size=img_size,
                                              min_size=min_size,
                                              max_size=max_size,
...
def convert_weights(param_layers, v='V1', debug=False):
    ...
    for layer in param_layers:
        ...
        if typ == 'innerproduct':
            ...
            weights_p = np.swapaxes(weights_p, 0, 3)
            weights_p = np.swapaxes(weights_p, 1, 2)
...
def load_weights(model, weights):
    for layer in model.layers:
        if weights.has_key(layer.name):
            ...
            print
            "Copied wts for layer:", layer.name
